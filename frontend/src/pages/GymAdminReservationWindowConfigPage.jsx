...
import { reservationWindowConfigApi } from '../api/client'
import { useAuth } from '../auth/AuthContext'
import { firstApiError } from '../utils/format'

export default function GymAdminReservationWindowConfigPage() {
  const { user } = useAuth()
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      setError('')
      try {
        const data = await reservationWindowConfigApi.get(orgId)
        if (!active) return
        setValue(String(data?.max_reservation_window_days ?? 21))
      } catch (apiError) {
        if (!active) return
  ...
  const submit = async (event) => {
    ...
    setSuccess('')
    try {
      const data = await reservationWindowConfigApi.update(orgId, {
        max_reservation_window_days: numericValue,
      })
      setValue(String(data?.max_reservation_window_days ?? numericValue))
  ...
  return (
    <div className="space-y-6">
      <DashboardHeader
        title="Gym Admin · Anticipación de reservas"
        subtitle="Define cuántos días hacia adelante pueden reservar clases los alumnos."
        ...
        {loading ? (
          ...
          <form onSubmit={submit} noValidate className="space-y-5">
            <div className="space-y-2">
              <label className="block text-sm font-semibold" htmlFor="max-reservation-window-days">
                Anticipación máxima para reservas
              </label>
              <p className="text-xs text-brand-muted">
                Los alumnos solo podrán reservar clases cuya fecha esté dentro de esta cantidad de días.
              </p>
              <div className="flex items-center gap-3">
                <input
                  id="max-reservation-window-days"
                  type="number"
                  step="1"
                  min="1"
                  max="366"

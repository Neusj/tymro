...
export default function IndividualConsultationsPage() {
  ...
  return <div className="space-y-6"><DashboardHeader title="Consultas individuales" subtitle="Unidades sin horario inicial; coordina y registra su realización." />
    ...
    {admin && <form onSubmit={assign} className="card-surface grid gap-3 p-5 md:grid-cols-2 xl:grid-cols-4"><select required value={form.student_id} onChange={e => setForm({ ...form, student_id: e.target.value })} className="rounded-xl border border-brand-line bg-black/30 px-3 py-2 text-sm text-brand-white"><option value="">Selecciona alumno</option>{users.filter(x => x.role === 'student').map(x => <option key={x.id} value={x.id}>{name(x)}</option>)}</select><select required value={form.professional_id} onChange={e => setForm({ ...form, professional_id: e.target.value })} className="rounded-xl border border-brand-line bg-black/30 px-3 py-2 text-sm text-brand-white"><option value="">Selecciona profesional</option>{users.filter(x => ['teacher', 'gym_admin'].includes(x.role)).map(x => <option key={x.id} value={x.id}>{name(x)}</option>)}</select><select required value={form.product_id} onChange={e => setForm({ ...form, product_id: e.target.value })} className="rounded-xl border border-brand-line bg-black/30 px-3 py-2 text-sm text-brand-white"><option value="">Producto de consulta</option>{products.map(x => <option key={x.id} value={x.id}>{x.name}</option>)}</select><input type="datetime-local" value={form.agreed_at} onChange={e => setForm({ ...form, agreed_at: e.target.value })} className="rounded-xl border border-brand-line bg-black/30 px-3 py-2 text-sm text-brand-white" /><button className="rounded-xl bg-brand-blue px-4 py-2 text-sm font-semibold text-white">Asignar cortesía o corrección</button></form>}
